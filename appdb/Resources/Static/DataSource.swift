...
public class DataSource: NSObject {
    ...
    public var sections: [StaticSection] {
        didSet {
            assert(Thread.isMainThread, "You must access Static.DataSource from the main thread.")
            refresh()
        }
    ...
    public init(tableView: UITableView? = nil, sections: [StaticSection]? = nil, tableViewDelegate: UITableViewDelegate? = nil) {
        assert(Thread.isMainThread, "You must access Static.DataSource from the main thread.")
    ...
    public func row(at point: CGPoint) -> StaticRow? {
    ...
    fileprivate func section(at index: Int) -> StaticSection? {
    ...
    fileprivate func row(at indexPath: IndexPath) -> StaticRow? {
    ...
    private func refreshTableSections(oldSections: [StaticSection]? = nil) {
        guard let tableView = tableView else { return }
