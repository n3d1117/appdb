...
extension API {
    ...
    static func getNews(limit: Int = 10, success:@escaping (_ items: [SingleNews]) -> Void, fail:@escaping (_ error: NSError) -> Void) {
        AF.request(endpoint, parameters: ["action": Actions.getNews.rawValue, "lang": languageCode, "limit": String(limit)], headers: headers)
    ...
    static func getNewsDetail(id: String, success:@escaping (_ item: SingleNews) -> Void, fail:@escaping (_ error: NSError) -> Void) {
        AF.request(endpoint, parameters: ["action": Actions.getNews.rawValue, "lang": languageCode, "id": id], headers: headers)
